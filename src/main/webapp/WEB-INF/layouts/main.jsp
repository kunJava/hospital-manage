...
<script type="text/javascript">
    ...
    function getSite() {
        try{
            var siteName = CookieUtil.getCookie("siteName");
            if (StringUtil.isNotNull(siteName)) {
                $("#choosed").text(siteName);
            }
            var siteId = CookieUtil.getSiteId();
            if (StringUtil.isNull(siteId)) {
                CookieUtil.setCookie("siteId", "abb6ed2c7436453cb6430c152cf6cf4d");
            }
            $.getJSON(trip_plat+"/appPlatInterface/getPlatSiteSetting").done(function(json) {
                if (json.msg == 'success') {
                    var data = json.data;
                    var html = "";
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var id = bean.id;
                        var name = bean.name;
//                         if(name == remote_ip_info["city"]){
//                          	$("#choosed").text(name);

//                          	CookieUtil.setCookie("siteId",id);
//                         }
                        html +='<a href="javascript:void();" lang="'+id+'">'+name+'</a>';
                    }
                    $(".combo-t").html(html);
                    $('.combo-t a').bind('click', function () {
                        var id = $(this).attr("lang");
                        var name = $(this).text();
                        $("#choosed").text(name);
                        CookieUtil.setCookie("siteName", name);
                        CookieUtil.setCookie("siteId", id);
                        window.location.reload();
                    });
                }
    ...
    //获取二维码
    function getErweima(obj,ad_id) {
        try{
            //站点ID
            var mall_siteId = CookieUtil.getSiteId();
            $.getJSON(ad_url+"?adAdsenseCode="+ad_id+"&pidSite="+mall_siteId).done(function (json) {
                if (json.msg == 'true') {
                    var head_url = json.url || '';
                    var map_arr = json.json;
                    if (map_arr.length > 0) {
                        var map = eval("(" + map_arr[0] + ")");
                        var image_id = map.image;
                        $("#"+obj).attr("src",head_url+image_id);
                    }
                }
            }, 'json');
        }catch(err){
            ExceptionUtil.catchError(err);
        }
    }
    //消息列表
    function toxiaoxiIndex(){
        var userId=CookieUtil.getUserId();
        $(document).an_dialog({
            id:'viewDialog',
            title : '查看信息详情列表',
            width : 500,
            height : 400,
            url : trip_plat+'/tMpInternalMessage/list?userId='+userId
        });
        getSite();
    }
    //软文方法
    function openInformationDialog(code,title){
//	code = encodeURI(code);
        $(document).an_dialog({
            id:'userRegDesDialog',
            title:title,
            width:800,
            height:680,
            url:'${base}/getInfoByCode?code='+code
        });
    }
</script>
</html>
