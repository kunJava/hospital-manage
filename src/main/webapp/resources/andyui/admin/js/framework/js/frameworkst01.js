(function ($, win) {
    ...
    */
    ...
    _module.setSideMenu = function(jsonurl, menuId){
        ...
        if(typeof jsonurl == "object"){
            ...
            if(topdata.length>0 && _options.api.systemlist == ""){
        ...
        _module.topnav.an_topnav({
            ...
            onClick:function(li){
                ...
                if(lidata.children && lidata.children.length>0){
                    if(lidata.chooseId){
                        _module.setSideMenu(lidata.children, lidata.chooseId)
                    }else{
    ...
    _module.setSystemList = function(){
        ...
        for(var i = 0; i < systemData.length; i++){
            ...
            if(frameworkData.system != "" && frameworkData.system == sd.id){
                // 默认选项
                sd.children?_module.setSideMenu(sd.children):null;
            }
        }
        systemUl.click(function(e){
            ...
            var li = t.parent();
            if(li.data("json").children && li.data("json").children.length > 0){
                _module.setSideMenu(li.data("json").children)
            }else{
                _module.sidemenu = $("#"+_options.id.sidemenu);
                _module.sidemenu.empty();
            }
            // li.data("json").children?_module.setSideMenu(li.data("json").children):null;
    ...
    */
    ...
    // 初始化数据加载 *********************
    _module.initTop = function(){
        var topData = [];
        andy.loaddata(_options.api.top,null, function(data, e){
