...
(function (andy) {
        ...
        var leftMenuStr = ' {{each list as value i}}'
            ...
            + '<a href="{{if list[i].children[j].url}}{{list[i].children[j].url}}{{else}}javascript:void(0){{/if}}" index="{{i}}_{{j}}" target="{{list[i].children[j].target||centerIframeID}}" class="rippler rippler-default">{{if list[i].children[j].icons}}<i class="iconfont classicon" title="{{list[i].children[j].text}}">{{list[i].children[j].icons}}</i>{{/if}}'
        ...
        var topMenuOverFlow = function () {
            ...
            getLastNode = function () {
                ...
                for (i = offsetCount; i < liList.length; i++) {
                    if (liList[i].offsetLeft+liList[i].offsetWidth > -ulBox.offsetLeft + absolutWidth) {
                        //上一个就是最后一个
            ...
            fixBoxWidth = function () {
                ...
                if (lastIndex !== liList.length - 1) {
                    // console.log(lastNode.offsetLeft,lastNode.offsetWidth,ulBox.offsetLeft)
                    try{
                        topmenu.width(lastNode.offsetLeft+lastNode.offsetWidth + ulBox.offsetLeft);
                    }catch(e){}
                }
            };
            ...
            if (ulBox.offsetWidth > absolutWidth) {
                ...
                rightBt.on("click", function () {
                    var left = -liList[lastIndex].offsetLeft - liList[lastIndex].offsetWidth;
        ...
        module.refreshPath = function (e, parent, mainUl) {
            ...
            //var indexs = event.target||e.getAttribute("index").split("_");
			...
			//el.getAttribute("index").split('_');
			
            var indexs = el.getAttribute("index").split("_");
            var i = 0, str = "";
         ...
         * @param onlyLeftMenu 是否只有左侧菜单 没有传为 false
         */
        module.topPartMenu = function (parent, centerIframeID, chooseCallback, onlyLeftMenu) {
            var str, leftLastUl = module.leftLastUl, lastA, lastCA, lastCCUL;
...
//                  $this.parent().addClass("activate");
			...
			////鼠标移出
            $('.nav-sub').bind('mouseleave',function (e) {
				module.clickLeftChild($(e.target.parentElement.parentElement).prev());
            });
