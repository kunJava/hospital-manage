    ...
    $.fn.extend({
    	an_uploadSWF:function(){
            ...
            if(funstyle != ""){
            	...
            	// 	// 高版本浏览器执行an_uploadSWF_h
	            // 	upload.an_uploadSWF_h(_options)
	            // 	return false;
	            // }
		        ...
		        var dialog_bottom = "<div class='u-p-l-bottom u-upload-bot' style = 'width:100%;height:40px;background:#EEE;position:absolute;bottom:0px;left:0px;_top:385px;_left:5px;_z-index:10000;'>"+
							          ...
							          "<a href='javascript:;' class='u-button-default success submit' style='float:left; margin-left:10px;'>确定</a>"+
							        "</div></div>";
		        ...
		        var openDialog = function(){
                	touchTarget.on("click", function(){
                		$(document).an_dialog({
	                        ...
	                        title: "文件上传管理",
	                        html: "<div class='f-p' style = 'height:325px;overflow-y:auto;'><ul class='u-upload-load'><li class='last'><img src='"+_options.imgUrl+"img1.jpg'></li></ul></div>"+dialog_bottom,
	                        width:675,
							height:430
	                    });
	                    dialog = $("#"+dialogId);
	                    ...
	                    submit.on("click", function(){
	                    	...
	                    	if(isUploadListComplete && _options.uploadUrl != "" && submit.isClick == false){
								...
								//   }
								// xmlhttp.open("POST",_options.uploadUrl,true);
								// xmlhttp.send();
            	...
            	// 设置图片大小
            	var setImgSize = function(imgPic, url){
            		if(!imgPic){
            			return false;
            		}
                ...
                var createUl = function(imgPic, data){
					...
					var li = "<li>"+imgPic+"<p>"+data.name+"</p></li>";
					showImg.append(li);

					var showPic = showImg.find("img");
					var previewPic = preview.find("img");

					if(showPic.attr("src") != previewPic.attr("src") && checkIsImg(data.type)){
						setImgSize(showPic, previewPic.attr("src"));
					}
                };
            	...
            	upload.bind(showFun, function(e, data){
					...
					if(dialog != ""){
						...
						if(isHaveSameFile(arr[0]) == false){
							...
							var liLength = dialog.find(".u-upload-load li").length;
							var touch = dialog.find(".last");
							if(isHaveSameFile(arr[0]) == false && liLength <= _options.filesCounts){
								var li = "<li filename ='"+data.name+"' >"+imgPic+"<p>"+data.name+"</p><span class='loading'><span class='speed' style='width:0'></span></span></li>";
						...
						if(liindex < _options.filesCounts){
							...
							if(isHaveSameFile(arr[0]) == false){
								...
								if(imgUrl){//移至该处，防止相同图片重复提交
									...
									setImgSize(imgPic, imgUrl);
								}
							}
						}
            		}else{
            			//完成回调
            			_options.uploadComplete(data);
            		...
            		if(dialog != "" && _options.files){
            			...
            			if(liindex < _options.filesCounts){
	        				...
	        				if(data.num == 100 && isHaveSameFile(arr[0]) == false){
								...
								file.append("<i class='iconfont'>&#xe62a;</i>");
	            				...
	            				// dialog.find(".u-upload-load [filename = '"+data.target.name+"']").
	            			}
	            		}
    	...
    	an_uploadSWF_h:function(){
            ...
            if(funstyle != ""){
            	...
            	var fileInput;
            	var fileId = upload_id+"_file";
            	var submitId = upload_id+"submit";
            	var postname = "iframeGetPage";
            	var fileInputStr = "<form enctype='multipart/form-data' method = 'post' target='"+postname+"' action = 'http://www.andyui.com/project/ui/do.php' class = 'f-hidden'><input type='file' id='"+fileId+"' name='file[]' /><input type = 'submit' id = '"+submitId+"'/></form>";
            	// 接收提交跳转
				touchTarget.after("<iframe name = '"+postname+"' class = 'f-hidden'></iframe>");
            	var toClickTarget = function(id){
            		// 模拟点击
				...
				// 单选多选初始化
                if(_options.files == false){
                	touchTarget.after(fileInputStr);
                	fileInput = $("#"+fileId);
                	fileInput[0].addEventListener('change', fileSelected, false);
                	touchTarget.on("click", function(){
                		toClickTarget(fileId)
                	})
                }else{
                	// if(_options.dialog == true){
                	// 	openDialog();
